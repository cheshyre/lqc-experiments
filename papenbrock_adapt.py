...
def main():
    ...
    if N==16:
        from papenbrock_ham import get_info
        ...
        from papenbrock_ham_24 import get_info
        gs = -72
    ...
    for i in range(max_iter):
        ...
        for op in pools:
            ...
            gradient = jax.grad(lambda x: vqe(x,operators_,Hamiltonian))(theta_)
            # print(gradient)
        ...
        pbar = tqdm.tqdm(range(30), desc="Optimizing")
        for ii in range(30):
            gradient = jax.grad(lambda x: vqe(x,operators,Hamiltonian))(theta)

            updates, opt_state = opt.update(gradient, opt_state)
            theta = optax.apply_updates(theta, updates)

            energy.append(vqe(theta,operators,Hamiltonian))
        ...
        plt.figure()
        plt.plot(energy)
        plt.savefig('lattice_energy_optimization.png')
        np.save('results/nuclear/energy.npy',energy)
        np.save('results/nuclear/indices.npy',indices)
